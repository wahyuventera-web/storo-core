...
- **Webhook Biteship pusat:** `src/app/api/webhooks/biteship/route.ts` menerima semua event tracking shipment (order.status, order.price, order.waybill_id) dari akun Biteship VenteraAI. Multi-tenant via lookup kolom `orders.shipping_tracking_number` (waybill/nomor resi, cross-store). Index: `idx_orders_shipping_tracking_number` (migration `009_index_shipping_tracking.sql` di storoengine). Env: `BITESHIP_WEBHOOK_SECRET`. URL di dashboard Biteship: `https://storo.id/api/webhooks/biteship`.
  - **Kontrak dengan storoengine:** webhook bergantung pada storoengine menyimpan `shipping_tracking_number` saat create shipping order (response Biteship `courier.waybill_id`). Jika null, lookup gagal dan event di-ignore. Lihat `storoengine/CLAUDE.md` bagian "Masalah yang Diketahui" untuk detail edge case.
  - **Tidak ada fallback by `biteship.order_id`** — keputusan desain: lookup murni by waybill untuk kesederhanaan dan performa (index langsung pada kolom, bukan JSONB path). Jika waybill hilang di payload (mis. event awal sebelum kurir allocated), webhook return `{status: "ignored"}` dengan warning log.
